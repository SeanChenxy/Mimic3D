...
def gen_interp_video(G, mp4: str, seeds, shuffle_seed=None, w_frames=60*4, kind='cubic', grid_dims=(1,1), num_keyframes=None, wraps=2, psi=1, truncation_cutoff=14, cfg='FFHQ', image_mode='image', gen_shapes=False, device=torch.device('cuda'), **video_kwargs):
    ...
    focal_length = 4.2647 if cfg.lower() != 'shapenet' else 1.7074 # shapenet has higher FOV
    ...
    print(f'Video save at {mp4}')
    os.makedirs(os.path.dirname(mp4), exist_ok=True)
    video_out = imageio.get_writer(mp4, mode='I', fps=60, codec='libx264', **video_kwargs)

    if gen_shapes:
        outdir = os.path.dirname(mp4) + '/interpolation_{}_{}/'.format(all_seeds[0], all_seeds[1])
...
def generate_images():
    ...
    print('Loading networks from "%s"...' % opts.resume)
    device = torch.device('cuda')
    with dnnlib.util.open_url(opts.resume) as f:
        G = legacy.load_network_pkl(f)['G_ema'].to(device) # type: ignore

    G.rendering_kwargs['depth_resolution'] = int(G.rendering_kwargs['depth_resolution'] * opts.sampling_multiplier)
    G.rendering_kwargs['depth_resolution_importance'] = int(G.rendering_kwargs['depth_resolution_importance'] * opts.sampling_multiplier)
    if opts.nrr is not None: G.neural_rendering_resolution = opts.nrr

    if opts.truncation_cutoff == 0:
        opts.truncation_psi = 1.0 # truncation cutoff of 0 means no truncation anyways
    if opts.truncation_psi == 1.0:
        opts.truncation_cutoff = 14 # no truncation so doesn't matter where we cutoff

    if opts.interpolate:
        output = os.path.join(outdir, 'gen/interpolation.mp4')
        gen_interp_video(G=G, mp4=output, bitrate='10M', grid_dims=opts.grid, num_keyframes=opts.num_keyframes, w_frames=opts.w_frames, seeds=opts.seeds, shuffle_seed=opts.shuffle_seed, psi=opts.truncation_psi, truncation_cutoff=opts.truncation_cutoff, cfg=opts.cfg, image_mode=opts.image_mode, gen_shapes=opts.shapes)
    else:
        for seed in opts.seeds:
            output = os.path.join(outdir, f'gen/{seed}.mp4')
            seeds_ = [seed]
            gen_interp_video(G=G, mp4=output, bitrate='10M', grid_dims=opts.grid, num_keyframes=opts.num_keyframes, w_frames=opts.w_frames, seeds=seeds_, shuffle_seed=opts.shuffle_seed, psi=opts.truncation_psi, truncation_cutoff=opts.truncation_cutoff, cfg=opts.cfg, image_mode=opts.image_mode)
