...
def save_image_grid(img, fname, drange, grid_size, board=None, seeds=None):
    lo, hi = drange
    ...
    if len(grid_size)==0:
        for i, s in enumerate(seeds):
            img_s = img[i].transpose(1,2,0)
            PIL.Image.fromarray(img_s, 'RGB').save(fname[:-4]+f'_{s}'+fname[-4:])
    else:
        gw, gh = grid_size
        _N, C, H, W = img.shape
        ...
        assert C in [1, 3]
        if board is not None and board[0] is not None:
            tfevent, board_name, n_iter = board
            tfevent.add_image(board_name, img.transpose(2, 0, 1), n_iter)
            return
...
def set_grad(module, free_param):
    if len(free_param) == 0:
        module.requires_grad_(True)
    else:
        for n, p in module.named_parameters():
            fr = [f in n for f in free_param]
            if any(fr):
...
def training_loop(
    ...
    D = None
    if D_kwargs.class_name is not None:
    ...
    if rank == 0:
        ...
        if D_patch is not None:
            misc.print_module_summary(D_patch, [img, None, 'image_raw'])
    ...
    for name, module, opt_kwargs, reg_interval in phases_asserts:
        if 'free_param' in opt_kwargs:
            free_param = opt_kwargs.pop('free_param')
            set_grad(module, free_param)
            opt_param = filter(lambda p: p.requires_grad, module.parameters())
            # opt_param = filter(lambda n, p: any([n in f for f in opt_kwargs.pop('free_param')]), module.named_parameters())
        else:
            free_param = []
            opt_param = module.parameters()
        if reg_interval is None:
            opt = dnnlib.util.construct_class_by_name(opt_param, **opt_kwargs) # subclass of torch.optim.Optimizer
            phases += [dnnlib.EasyDict(name=name+'both', module=module, opt=opt, interval=1, free_param=free_param)]
            ...
            phases += [dnnlib.EasyDict(name=name+'main', module=module, opt=opt, interval=1, free_param=free_param)]
            phases += [dnnlib.EasyDict(name=name+'reg', module=module, opt=opt, interval=reg_interval, free_param=free_param)]
    for phase in phases:
    ...
    if rank == 0:
        ...
        save_image_grid(images, os.path.join(run_dir, 'imgs/reals.png'), drange=[0,255], grid_size=grid_size, board=None) # [stats_tfevents, 'real/img', 0]
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            set_grad(phase.module, phase.free_param)
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            out = [G_ema(z=z, c=c, noise_mode='const', patch_scale=loss_kwargs['patch_scale']) for z, c in zip(grid_z, grid_c)]
            ...
            save_image_grid(images, os.path.join(run_dir, f'imgs/fakes{cur_nimg//1000:06d}.png'), drange=[-1,1], grid_size=grid_size, board=None) # [stats_tfevents, 'fake/img', cur_nimg//1000]
            save_image_grid(images_raw, os.path.join(run_dir, f'imgs/fakes{cur_nimg//1000:06d}_raw.png'), drange=[-1,1], grid_size=grid_size, board=None) # [stats_tfevents, 'fake/raw', cur_nimg//1000]
            save_image_grid(images_depth, os.path.join(run_dir, f'imgs/fakes{cur_nimg//1000:06d}_depth.png'), drange=[images_depth.min(), images_depth.max()], grid_size=grid_size, board=None) # [stats_tfevents, 'fake/depth', cur_nimg//1000]
            if 'patch_image' in out[0]:
                patch_images = torch.cat([o['patch_image'].cpu() for o in out]).numpy()
                save_image_grid(patch_images, os.path.join(run_dir, f'imgs/fakes{cur_nimg//1000:06d}_patch.png'), drange=[-1,1], grid_size=grid_size, board=None) # [stats_tfevents, 'fake/patch', cur_nimg//1000]
            if 'patch_image_raw' in out[0]:
                patch_images_raw = torch.cat([o['patch_image_raw'].cpu() for o in out]).numpy()
                save_image_grid(patch_images_raw, os.path.join(run_dir, f'imgs/fakes{cur_nimg//1000:06d}_patch_raw.png'), drange=[-1,1], grid_size=grid_size, board=None) # [stats_tfevents, 'fake/patch_raw', cur_nimg//1000]
            if 'patch_image_gr' in out[0]:
                patch_images = torch.cat([o['patch_image_gr'].cpu() for o in out]).numpy()
                save_image_grid(patch_images, os.path.join(run_dir, f'imgs/fakes{cur_nimg//1000:06d}_patch_gr.png'), drange=[-1,1], grid_size=grid_size, board=None) # [stats_tfevents, 'fake/patch_gr', cur_nimg//1000]
        ...
        # Evaluate metrics.
        if (snapshot_data is not None) and (len(metrics) > 0) and cur_nimg > 100:
            if rank == 0:
                print(run_dir)
                print('Evaluating metrics...')
            for metric in metrics:
                result_dict = metric_main.calc_metric(metric=metric, G=snapshot_data['G_ema'], G_kwargs=metric_kwargs,
