...
class SingleDiscriminator(torch.nn.Module):
    ...
    def forward(self, img, c, img_name='image', img_raw_name=None, update_emas=False, retfeat=False, **block_kwargs):
        img = img[img_name]
...
class DualDiscriminator(torch.nn.Module):
    ...
    def forward(self, img, c, img_name='image', img_raw_name='image_raw', update_emas=False, retfeat=False, **block_kwargs):
        if img[img_raw_name].shape[-1] == img[img_name].shape[-1]:
            image_raw = img[img_raw_name]
        else:
